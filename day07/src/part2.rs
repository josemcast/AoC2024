fn concatenate(prefix: i64, comp: i64) -> i64 {
    let result = format!("{}{}", prefix, comp);
    result.parse().unwrap()
}
fn is_calib(test: i64, prefix: i64, v: &[i64]) -> bool {
    if v.len() < 2 {
        if (prefix + v[0]) == test {
            ...
            return true;
        } else if (concatenate(prefix, v[0])) == test {
            return true;
        }
        return false;
    }

    is_calib(test, prefix + v[0], &v[1..])
        || is_calib(test, prefix * v[0], &v[1..])
        || is_calib(test, concatenate(prefix, v[0]), &v[1..])
}
